    ...
    if (sync_project_deps.length > 0) {
      ...
      for (const name of sync_project_deps) {
        ...
        if (existsSync(global_path)) {
          ...
          modules.push({ name, path: global_path });
      ...
      console.log(chalk.bold.yellow(`[ UPDATED VERSIONS OF LINKED MODULES IN LOCAL PACKAGE.JSON ]`));
      console.log("");
      // END
    } else {
      console.log(chalk.bold.white("[ LIST IS EMPTY ]"));
      console.log("");
    }
  } catch (error) {
