...
- Проверка конфигурации на избыточные, недостающие или некорректные свойства. Имеется в виду build.config.json, который содержит полный список полей доступных для конфигурации, ни больше и ни меньше. Если указать какие-то неизвестные поля для rearguard, то вы получите сообщение о существовании полей, которые не используются. Если забыть указать
необходимые поля то вы увидете предупреждение, что полей нехватает и какие текущие настройки применились. Если указать 
