...
- Проверка конфигурации на избыточные, недостающие или некорректные свойства. Имеется в виду build.config.json, который содержит полный список полей доступных для конфигурации, ни больше и ни меньше. Если указать какие то неизвестные поля то вы получите сообщение о том что существуют поля которые не используются. Если забыть указать
